<template>
	<div class="partake">
		...
		<div class="goodsdetail" @click="getGoods(curObj.productId)">
			<div class="goods-l">
		...
		<div class="name">
			<ul class="name-list">
				...
				<li v-show="curObj.status==0"><img :src="quesImg"/></li>
			</ul>
		</div>
		<div class="bottom">
		...
		<div class="content">
			<ul>
				<li v-for='item in listObj' @click="goBuy(item.productId)">
...
<script>
	...
	export default {
		...
		methods:{
			...
			//点击品牌特卖
			goFive(){
				window.location.href=API_HOST+'ol/weixin/index/indexRecommendBrand?memberId='+this.$route.query.memberId
			},
			getList(){
			...
			getListBack(data){
				...
				if(this.$route.params.id=='undefined' || this.$route.params.id==undefined){
					...
					this.$toast('请刷新页面，再分享');
				}else{
					this.shareData.share=false;
				}
				//this.shareData.url="http://ol-site.olquan.com/weixin/auth?recId="+this.$route.query.memberId+"&view="+encodeURIComponent(CUR_URLBACK+'share/share?id='+this.$route.params.id);
				this.shareData.url="http://test-mobile.olquan.cn/weixin/auth?recId="+this.$route.query.memberId+"&view="+encodeURIComponent(CUR_URLBACK+'share/share?id='+this.$route.params.id);
			...
			shareOrder(){
				...
				//window.location.href='http://ol-h5.olquan.cn/share/share/id/'+this.$route.params.id+'?memberId='+this.$route.query.memberId+'&togetherId='+1+'&num='+1
			},
			colseShare(){
			...
			buyGoods(id){
				...
				//window.location.href='http://test-mobile.olquan.cn/ol/confirmOrder1.html?num='+this.$route.query.num+'&'+'normalId='+'&'+'memberId='+this.$route.query.memberId+'&'+'togetherId='+this.$route.query.togetherId +'&'+'togetherRecordId='+this.$route.params.id
			},
			//微信分享 

	...
	<div class="mine">
		<div class="member" v-show="listObj.levelCode!='white' && listObj.levelCode!='golden'">
			<div class="headImg">
				...
				<div class="dateTime" @click="getPinkuse">
					<p>续费</p>
		...
		<div class="pink-invio">
			...
			<div class="jinOwn">
				<ul>
					...
					<li style="border-right: 0.01rem solid #ddd;border-bottom: 0.01rem solid #ddd;">
						<img :src="tuifenImg"/>
						<span>推荐赚积分</span>
						<span style="margin-top: .10rem;">3000</span>
					</li>
			...
			<div class="pinkOwn" v-show="showFen">
				...
				<span style="margin: 0 .30rem; font-weight: 700;">粉领专享</span>
		...
		<div class="cent" v-show="showFen">
			<ul >
				<li v-for='item in curObj' @click="goBuy(item.productId)">
					...
					<p style="font-size: .34rem;color: #fc469a; line-height: .32rem;">￥{{item.price}}
						<span style="font-weight: normal;text-align:center;width:.60rem; height:.32rem;font-size:.24rem;display: inline-block;color:#fff;background: #fc469a;">粉领</span>
					</p>
					<p class="botMoney">
		...
		methods:{
			...
			kaiPinkuse(){
				if(tsApp.getClientBrowser()=='wx'){
					window.location.href=API_HOST+'weixin/member/openStore?&memberId='+this.$route.query.memberId
				}else{
					if(this.isAndroid){
						OLquan.toProduct("1")
					}
			...
			goIndex(){
				if(tsApp.getClientBrowser()=='wx'){
					window.location.href=API_HOST+'ol/weixin/index/modelVM?code=index&memberId='+this.$route.query.memberId
				}else{
					if(this.isAndroid){
						OLquan.toProduct("3")
					}
...
//				console.log('是否是iOS：'+this.isiOS);
			...
			getPinkuse(){
				if(tsApp.getClientBrowser()=='wx'){
					window.location.href=API_HOST+'weixin/member/renewConfirmOrder';
				}else{
					if(this.isAndroid){
						OLquan.toProduct("8")
					}
...
//					window.location.href="邀请粉领";//点击去邀请4
			...
			getInvite(){
				if(tsApp.getClientBrowser()=='wx'){
					window.location.href=API_HOST+'weixin/member/memberTwoCode2?memberId='+this.$route.query.memberId
				}else{
					if(this.isAndroid){
						OLquan.toProduct("6")
					}
			...
			getDetailOne(){
				if(tsApp.getClientBrowser()=='wx'){
					window.location.href=API_HOST+'weixin/freeUse/freeUse?type=3&memberId='+this.$route.query.memberId
				}else{
					if(this.isAndroid){
						OLquan.toProduct("2")
					}
			...
			gotoJifen(){
				//console.log(CUR_URLBACK+'integral/uesget?memberId='+this.$route.query.memberId)
			...
			getDetailBack(data){
				...
				this.time=this.listObj.endStoreTime.split(" ")[0];
				//console.log(this.time)
				//this.shareData.url="http://ol-site.olquan.com/weixin/auth?recId="+this.$route.query.memberId+"&view="+encodeURIComponent(CUR_URLBACK+'mine/index');
				this.shareData.url="http://test-mobile.olquan.cn/weixin/auth?recId="+this.$route.query.memberId+"&view="+encodeURIComponent(CUR_URLBACK+'mine/index');

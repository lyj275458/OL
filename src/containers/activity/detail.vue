...
//					notNextTick: true,
		...
		methods:{
			...
			getFirstBack(data){
				...
				//this.shareData.url="http://ol-site.olquan.com/weixin/auth?recId="+this.$route.query.memberId+"&view="+encodeURIComponent(CUR_URLBACK+'activity/detail?code='+this.$route.query.code);
				this.shareData.url="http://test-mobile.olquan.cn/weixin/auth?recId="+this.$route.query.memberId+"&view="+encodeURIComponent(CUR_URLBACK+'activity/detail?code='+this.$route.query.code);
				//console.log(this.urlList)
...
//					this.$store.state.ajaxObj.comAjax(this.$store.state.ajaxObj.API.findProductByContentId,data,this.moreFun);
//				}, 500);
//				
